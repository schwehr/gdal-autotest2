...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
  const char kFilename[] = "/vsimem/a.tar";
  ...
  //   /vsitar/vsimem/a
  const char kFilenameTar[] = "/vsitar//vsimem/a.tar";

  WithQuietHandler error_handler;

  char **files_csl = VSIReadDirRecursive(kFilenameTar);
  const std::vector<std::string> files = autotest2::CslToVector(files_csl);
  CSLDestroy(files_csl);

  for (const auto &filename : files) {
    std::string filepath = absl::StrCat(kFilenameTar, "/", filename);
